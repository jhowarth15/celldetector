...
class SettingWindow(object):
    def __init__(self, max_num_features, num_frames, mser_image):
        ...
        for i in range(max_num_features):
            self.features_vars.append(0)

        # default values
        for j in [0, 1, 3, 6, 7, 9, 15, 21, 23]:
            self.features_vars[j] = 1
    ...
    def get_forest_opts(self):
        return [300, 3, 0.5] #[self.num_trees_scale.get(), self.depth_tree_scale.get(), self.percentage_feats_scale.get()]

    def get_low_thresh(self):
        return 0.3 #self.low_thresh_scale.get()

    def get_high_thresh(self):
        return 0.7 #self.high_thresh_scale.get()

    def get_dots_distance(self):
        return 6 #int(self.dots_distance_scale.get())
    ...
    def get_selection_mask(self):
        ...
        for i, var in enumerate(self.features_vars):
            selection_mask[i] = self.features_vars[i]
